...
static int GMS_Summon(int boss, const char[] ability_name, float position[3])
{
	...
	for(; loadout<99; loadout++)
	{
		...
		if(GetArgI(boss, ability_name, temp, (loadout*100)+2, -999) == -999)
			break;
	}
	...
	TFClassType class = view_as<TFClassType>(GetArgI(boss, ability_name, temp, (loadout*100)+4, view_as<int>(TFClass_Unknown)));

	FormatEx(temp, MAX_ABILITY_LENGTH, "boss%i", loadout+1);
	BossType type = view_as<BossType>(GetArgI(boss, ability_name, temp, (loadout*100)+5, 1));
	...
	bool crits = view_as<bool>(GetArgI(boss, ability_name, temp, (loadout*100)+10, 0));
	...
	for(int i; i<GMS_WEAPONS; i++)
	{
		...
		index[i] = GetArgI(boss, ability_name, temp, (loadout*100)+(i*10)+12);
	...
	for(int i; i<dead && i<amount; i++)
	{
		...
		FF2Player(clone).SetPropAny("bIsMinion", true);
		FF2Player(clone).ForceTeamChange(bossTeam);
...
public Action GMS_BombThink(Handle timer, int ref)
{
	...
	if(!validEntity || !GMS_BombEnabled || (!validClient && GMS_BombTimer<GetGameTime()))
	{
		...
		if(FF2_GetRoundState()!=1 || !GME_Enabled || !FF2_RandomSound(GME_BOMBDEAD, sound, MAX_SOUND_LENGTH, GME_Boss))
			return Plugin_Stop;
...
public Action GMS_Flag(int client, int type)
{
	...
	if(!cvar)
	{
		cvar = FindConVar("ff2_point_delay");
		if(cvar && ff2_gm.iLivingReds > cvar.IntValue)
...
void GMS_Death(int client, int flags, bool clean)
{
	...
	{
		GMS_Owner[client] = -1;
		FF2Player(client).ForceTeamChange(VSH2Team_Red);
		if(!clean)
...
{
	if(FF2_GetRoundState()!=1 || !IsValidClient(client) || GMS_BossType[client]==Boss_None)
		return Plugin_Continue;
	...
	if(bIsTelefrag)
	{
		...
		damage = (isBossAttacker ? float(GMS_Health[client]) : 9001.0);
		return Plugin_Changed;
	}
	return Plugin_Continue;
}
...
{
	if(FF2_GetRoundState()!=1 || !IsValidClient(client) || GMS_BossType[client]!=Boss_Buster)
		return Plugin_Continue;
...
void GME_Setup(int client, int boss)
{
	...
	if(classname[0])
	{
		...
		SpawnWeapon(client, classname, GetArgI(boss, GME_NAME, "index", 2), 101, 5, attributes, view_as<bool>(GetArgI(boss, GME_NAME, "visible", 4)));
	}
...
void GMR_Setup(int client, int boss)
{
	...
	GMR_Hide[team] = GetArgI(boss, GMR_NAME, "hide", 3, 1);
...
void GMA_Setup(int client, int boss)
{
	...
	if(classname[0])
	{
		...
		SpawnWeapon(client, classname, GetArgI(boss, GMA_NAME, "index", 2), 101, 5, attributes, view_as<bool>(GetArgI(boss, GMA_NAME, "visible", 4)));
	}
	...
	if(GMR_Team != TFTeam_Spectator)
	{
		...
		GMR_Hide[team] = GetArgI(boss, GMA_NAME, "hide", 13, 1);
