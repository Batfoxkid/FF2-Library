...
#define MAX_MAPS 100
#define MAX_RECTS 10
bool TS_EnabledThisMap;
int BossTeam = -1;
int TS_LightColor;
float TS_BaseDamage;
float TS_BossDamage;
float TS_BaseExp;
float TS_MinAmmoLoss;
float TS_MaxAmmoLoss;
float TS_MedigunLoss;
int TS_BossGracePeriod;
char TS_MercMSG[MAX_CENTER_TEXT_LENGTH];
char TS_BossMSG[MAX_CENTER_TEXT_LENGTH];
char TS_SlayCenterText[MAX_CENTER_TEXT_LENGTH];
char TS_SlayGlobalMessage[MAX_CENTER_TEXT_LENGTH];

// map-specific
int TSM_RectCount;
float TSM_NextCheckAt;
float TSM_Coords[MAX_RECTS][2][3];
float TSM_LightPoint[MAX_RECTS][3];
float TSM_Luminosity[MAX_RECTS];
bool TSM_IsSlay[MAX_RECTS];

// player specific
int TSP_ViolationCount[MAX_PLAYERS_ARRAY];
int TSP_NoClipGraceCount[MAX_PLAYERS_ARRAY]; // needed to prevent accidental slay with noclip rages
...
public float TS_ReadFloat(Handle kv, const char[] structName, const char[] keyName, float defaultValue)
{
	KvRewind(kv);
	if (!KvJumpToKey(kv, structName))
	{
...
public void TS_ReadString(Handle kv, const char[] structName, const char[] keyName, char[] str, int length)
{
	KvRewind(kv);
	if (!KvJumpToKey(kv, structName))
	{
...
public void OnMapStart()
{
	...
	static char thisMap[PLATFORM_MAX_PATH];
	GetCurrentMap(thisMap, PLATFORM_MAX_PATH);
	for (int mapIdx = 0; mapIdx < MAX_MAPS; mapIdx++)
	{
		static char mapN[8];
		Format(mapN, sizeof(mapN), "map%d", mapIdx);
		KvRewind(kv);
		static char testMap[PLATFORM_MAX_PATH];
		...
		if (!IsEmptyString(testMap) && StrContains(thisMap, testMap) == 0)
		{
			...
			static char twoRectsStr[SIX_FLOAT_WIDTH];
			static char oneRectStr[THREE_FLOAT_WIDTH];
			TSM_RectCount = 0;
			for (int rectIdx = 0; rectIdx < MAX_RECTS; rectIdx++)
			{
				static char rectN[9];
				Format(rectN, sizeof(rectN), "rect%d", rectIdx);
				static char lightN[10];
				Format(lightN, sizeof(lightN), "light%d", rectIdx);
				static char luminosityN[15];
...
public bool TS_ParseCollisionRect(float coords[2][3], const char[] twoRectsStr)
{
	static char splitRectsStr[2][THREE_FLOAT_WIDTH];
...
public bool TS_ParsePoint(float coords[3], const char[] splitRectsStr)
{
	static char splitFloatsStr[3][LOGICAL_FLOAT_WIDTH];
...
public void TS_RoundStart(Handle event, const char[] name, bool dontBroadcast)
{
	...
	// spawn light objects
	for (int rectIdx = 0; rectIdx < TSM_RectCount && rectIdx < MAX_RECTS; rectIdx++)
	{
		if (TSM_LightPoint[rectIdx][0] == OFF_THE_MAP[0] || TSM_Luminosity[rectIdx] <= 0.0)
			continue; // invalid
	
		int dynLight = CreateEntityByName("light_dynamic");
...
public void OnGameFrame()
{
	...
	if (curTime >= TSM_NextCheckAt)
	{
		...
		for (int clientIdx = 1; clientIdx < MAX_PLAYERS; clientIdx++)
		{
			...
			if (GetEntProp(clientIdx, Prop_Send, "movetype") == view_as<any>(MOVETYPE_NOCLIP))
			{
		...
		for (int clientIdx = 1; clientIdx < MAX_PLAYERS; clientIdx++)
		{
			if (violators[clientIdx])
			{
				...
				int actualViolations = (GetClientTeam(clientIdx) == BossTeam) ? (TSP_ViolationCount[clientIdx] - TS_BossGracePeriod) : TSP_ViolationCount[clientIdx];
				if (actualViolations > 0)
				{
					...
					if (GetClientTeam(clientIdx) != BossTeam)
					{
						...
						float ammoDrain = TS_MaxAmmoLoss + ((TS_MinAmmoLoss - TS_MaxAmmoLoss) * (1.0 - (min(20, actualViolations) / 20.0)));
					...
					float damage = GetClientTeam(clientIdx) == BossTeam ? TS_BossDamage : TS_BaseDamage;
					if (TS_BaseExp > 1.0)
					...
					if (GetClientTeam(clientIdx) == BossTeam)
					{
						...
						int attacker = FindRandomPlayer(false);
